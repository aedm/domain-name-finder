...
use std::fs::File;
use std::io::Write;
use tools::{download_stream_to_file, fetch_json, get_aws_s3_client, get_env, send_request};
...
async fn get_all_files_from_s3_bucket(
...
async fn get_latest_s3_zone_date(
    client: &aws_sdk_s3::Client,
    bucket_name: &str,
...
async fn main() -> Result<()> {
    ...
    let client = get_aws_s3_client().await;
    let s3_bucket = get_env("S3_BUCKET_NAME")?;

    let latest = get_latest_s3_zone_date(&client, &s3_bucket).await?;
    println!("{latest:?}");
    Ok(())
}
